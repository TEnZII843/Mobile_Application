...
public class ActivityInputOutputTest {
    ...
    public void useAppContext() {
        ...
        assertEquals("bt.edu.todo_4", appContext.getPackageName());
    }
    @Rule
    ...
    public void textInputOutput() {
        onView(withId(R.id.editText_main)).perform(
                typeText(message),closeSoftKeyboard());
        onView(withId(R.id.button_main)).perform(click());
        onView(withId(R.id.editText_second)).perform
                (typeText(reply),closeSoftKeyboard());
        onView(withId(R.id.button_second)).perform(click());
    }
